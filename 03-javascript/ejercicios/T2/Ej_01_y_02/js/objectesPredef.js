...
if (titul.length >= 3 && titul.length <= 10) {
  if (barra === "YES") {
    ...
    wind.onload = function () {
      ...
      let hora = new Date();
      let dateElement = wind.document.getElementById("date");
      dateElement.textContent = hora.toLocaleTimeString();
    };
  } else {
    alert("se abrira una nueva ventana sin barras de navegación");
    let h = Math.random() * 50;
    let w = Math.random() * 400;
    let wind = window.open(
      "../Ej_06/finestra.html",
